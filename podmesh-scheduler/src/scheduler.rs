...
impl Scheduler {
    ...
    pub fn schedule_workload(
        ...
        let available_candidates = Self::collect_available_candidates(candidates);
        ...
        let actual_required_count = self.clamp_required_count(required_count);
    ...
    fn select_random(
        ...
        let mut indices: Vec<_> = available_candidates.iter().map(|(idx, _)| *idx).collect();
    ...
    fn clamp_required_count(&self, requested: usize) -> usize {
        self.config
            .max_candidates
            .map(|max| max.min(requested))
            .unwrap_or(requested)
    }

    fn collect_available_candidates<'a>(
        candidates: &'a [NodeCandidate],
    ) -> Vec<(usize, &'a NodeCandidate)> {
        candidates
            .iter()
            .enumerate()
