...
impl WorkloadManager {
    ...
    fn emit_event(&self, event: WorkloadEvent) {
        if let Some(sender) = &self.event_sender {
            if sender.send(event).is_err() {
                warn!("Dropping workload event because receiver is gone");
            }
        }
    }

    fn store_deployment(&self, deployment: DeploymentStatus) {
        self.deployments
            .write()
            .insert(deployment.workload_id.clone(), deployment);
    }

    fn fetch_deployment(&self, workload_id: &str) -> WorkloadManagerResult<DeploymentStatus> {
        self.deployments
            .read()
            .get(workload_id)
            .cloned()
    ...
    fn remove_deployment_entry(&self, workload_id: &str) -> Option<DeploymentStatus> {
        self.deployments.write().remove(workload_id)
    }

    fn fetch_engine(&self, name: &str) -> WorkloadManagerResult<Arc<dyn RuntimeEngine>> {
        let registry = self.runtime_registry.read();
        registry
            .get_engine(name)
    ...
    async fn select_runtime_engine(
        &self,
    ) -> WorkloadManagerResult<(String, Arc<dyn RuntimeEngine>)> {
        let engine_name = self.get_runtime_engine().await?;
        let engine = self.fetch_engine(&engine_name)?;
    ...
    pub async fn deploy_workload(
        ...
        let (engine_name, engine) = self.prepare_engine_for_manifest(manifest_content).await?;
        ...
        let workload_info =
            Self::deploy_via_engine(&engine, manifest_id, manifest_content, config).await?;
        ...
        let mut final_status =
            Self::build_deployment_status(&workload_info, manifest_id, &engine_name, config);

        self.store_deployment(final_status.clone());

        if self.should_attempt_provider_announcement()
            && self
                .try_announce_provider(swarm, manifest_id, &engine_name)
                .await
        {
            final_status.announced_as_provider = true;
            self.store_deployment(final_status.clone());
            self.emit_event(WorkloadEvent::ProviderAnnounced {
                manifest_id: manifest_id.to_string(),
                peer_id: *swarm.local_peer_id(),
            });
        }

        self.emit_event(WorkloadEvent::WorkloadDeployed {
            workload_id: workload_info.id,
            manifest_id: manifest_id.to_string(),
    ...
    pub async fn remove_workload(
        ...
        let deployment_status = self.fetch_deployment(workload_id)?;

        let engine = self.fetch_engine(&deployment_status.runtime_engine)?;
        ...
        if deployment_status.announced_as_provider {
                ...
                .stop_providing(&deployment_status.manifest_id)
            {
                Ok(()) => {
                    ...
                    self.emit_event(WorkloadEvent::ProviderStopped {
        ...
        self.remove_deployment_entry(workload_id);

        self.emit_event(WorkloadEvent::WorkloadRemoved {
    ...
    pub async fn remove_workloads_by_manifest_id(
        ...
        let workloads_to_remove = self.workloads_for_manifest(manifest_id);
    ...
    pub async fn get_workload_status(
        ...
        let engine = self.fetch_engine(&deployment_status.runtime_engine)?;
        let fresh_result = engine.get_workload_status(workload_id).await;
        ...
        match fresh_result {
            Ok(workload_info) => {
                ...
                self.store_deployment(updated_status.clone());
    ...
    pub async fn list_workloads_by_peer(&self, local_peer_id: &str) -> Vec<DeploymentStatus> {
        let deployments_snapshot: Vec<_> = {
            let deployments = self.deployments.read();
            deployments.values().cloned().collect()
        };

        let Some(registry_guard) = crate::workload_integration::get_global_runtime_registry().await
        else {
            return Vec::new();
        };
        ...
        for deployment in deployments_snapshot {
            if let Some(engine) = registry.get_engine(&deployment.runtime_engine) {
                work_items.push((deployment, engine));
            }
        }
        ...
        for (deployment, engine) in work_items {
            if let Ok(workload_info) = engine.get_workload_status(&deployment.workload_id).await {
                let matches_peer = workload_info
                    .metadata
                    .get("local_peer_id")
    ...
    pub async fn export_workload_manifest(
        ...
        let deployment_status = self.fetch_deployment(workload_id)?;
        let engine = self.fetch_engine(&deployment_status.runtime_engine)?;
        let manifest_bytes = engine.export_manifest(workload_id).await?;
            ...
            tokio::spawn(async move {
                ...
                loop {
                    ...
                    let workload_ids: Vec<String> = {
                        let deps = deployments_clone.read();
    ...
    async fn get_runtime_engine(&self) -> WorkloadManagerResult<String> {
        if let Some(ref preferred) = self.config.preferred_runtime {
            if let Ok(engine) = self.fetch_engine(preferred) {
        ...
        let default_name = {
            let registry = self.runtime_registry.read();
            registry.default_engine_name().map(|name| name.to_string())
        };

        if let Some(engine_name) = default_name {
            if let Ok(engine) = self.fetch_engine(&engine_name) {
                if engine.is_available().await {
    ...
    fn should_attempt_provider_announcement(&self) -> bool {
        self.config.announce_as_provider
    }

    async fn prepare_engine_for_manifest(
        &self,
        manifest_content: &[u8],
    ) -> WorkloadManagerResult<(String, Arc<dyn RuntimeEngine>)> {
        let (engine_name, engine) = self.select_runtime_engine().await?;
        if self.config.validate_manifests {
            self.validate_manifest_content(&engine, manifest_content)
                .await?;
        }
        Ok((engine_name, engine))
    }

    async fn deploy_via_engine(
        engine: &Arc<dyn RuntimeEngine>,
        manifest_id: &str,
        manifest_content: &[u8],
        config: &DeploymentConfig,
    ) -> WorkloadManagerResult<WorkloadInfo> {
        engine
    ...
    fn build_deployment_status(
        ...
        DeploymentStatus {
            ...
            manifest_id: manifest_id.to_string(),
            status: workload_info.status.clone(),
            runtime_engine: engine_name.to_string(),
    ...
    async fn try_announce_provider(
        &self,
        swarm: &mut Swarm<MyBehaviour>,
        manifest_id: &str,
        engine_name: &str,
    ...
    fn workloads_for_manifest(&self, manifest_id: &str) -> Vec<String> {
        self.deployments
            .read()
            .iter()
            .filter_map(|(workload_id, deployment)| {
                (deployment.manifest_id == manifest_id).then(|| workload_id.clone())
            })
            .collect()
    }
}
