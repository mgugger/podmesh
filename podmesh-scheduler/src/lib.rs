...
pub struct Cli {
    ...
    /// Directory to store machine keypair (default: /etc/podmesh/machine)
    #[arg(long, default_value = "/etc/podmesh/machine")]
    pub key_dir: String,
...
impl Default for Cli {
    fn default() -> Self {
        Self {
            ...
            api_socket: Some("/run/podmesh/host.sock".to_string()),
            key_dir: "/etc/podmesh/machine".to_string(),
...
pub async fn start_machine(cli: Cli) -> anyhow::Result<Vec<tokio::task::JoinHandle<()>>> {
    ...
    let _keypair = if cli.ephemeral {
        ...
        // try to create and set secure perms; on failure, fall back to $HOME/.podmesh
        ...
        if let Err(e) = ensure_dir(&key_path) {
            log::warn!(
                "could not create/set permissions for {}: {}. Falling back to $HOME/.podmesh",
                key_path.display(),
                e
            );
    ...
    let (mut swarm, topic, peer_rx, peer_tx) = podmesh_p2p::setup_libp2p_node(
    ...
    let (control_tx, control_rx) =
        tokio::sync::mpsc::unbounded_channel::<podmesh_p2p::control::Libp2pControl>();

    // Set the global control sender for distributed operations
    podmesh_p2p::set_control_sender(control_tx.clone());
    ...
    let libp2p_handle = tokio::spawn(async move {
        ...
        if let Err(e) = podmesh_p2p::start_libp2p_node(swarm, topic, peer_tx, control_rx).await {
