...
pub struct RuntimeRegistry {
    engines: HashMap<String, Arc<dyn RuntimeEngine>>,
...
impl RuntimeRegistry {
    ...
    pub fn register(&mut self, engine: Arc<dyn RuntimeEngine>) {
    ...
    pub fn get_engine(&self, name: &str) -> Option<Arc<dyn RuntimeEngine>> {
        self.engines.get(name).cloned()
    }

    /// Get the default runtime engine
    pub fn get_default_engine(&self) -> Option<Arc<dyn RuntimeEngine>> {
        self.default_engine
            .as_ref()
...
pub async fn create_default_registry() -> RuntimeRegistry {
    ...
    registry.register(Arc::new(engines::PodmanEngine::new()));
