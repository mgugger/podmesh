...
`podmesh-sidecar` is injected into each workload pod by the selected `podmesh-agent` after the
agent decrypts and verifies the owner-signed deployment grant.
It is the mesh agent that handles all inbound and outbound network traffic for the workload.

It runs as an additional container injected into the workload pod manifest by the selected agent.
It reads its configuration from a file or environment variable generated by that agent.
...
```
manifest_id          — full namespace-scoped 256-bit workload ID
manifest_b64         — base64-encoded full manifest payload
owner_public_key_b64 — (optional) base64 Ed25519 owner pubkey
bootstrap_peer       — multiaddr of a DHT bootstrap peer (set by agent configuration)
```
...
The sidecar runs a libp2p node using authenticated QUIC/TLS transport plus the Podmesh application
handshake with these protocols:
...
- The sidecar trusts `SidecarMetadata` generated by the selected agent from an owner-signed,
  encrypted deployment grant. A process with write access to the sidecar environment can still
  tamper with metadata; that process is already inside the selected execution trust boundary.
...
```
Agent decrypts the workload and injects SidecarMetadata into the sidecar environment
  ▼
