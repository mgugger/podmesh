...
- [x] Add `podctl cert grant-proxy` command: fetch proxy key material, sign NodeCert with owner keypair, POST to proxy `/api/v1/node_cert`
- [x] Add `POST /api/v1/node_cert` REST endpoint to proxy: verify cert fields (peer_id match, role, expiry, sig), store keyed by owner_pubkey
- [ ] Persist proxy NodeCert store durably across restarts
