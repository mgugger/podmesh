...
impl ApiClient {
        ...
        if response_bytes.len() > 100 {
            ...
            if let Ok(envelope) = protocol::machine::root_as_envelope(&response_bytes) {
                if let Some(payload) = envelope.payload() {
                    ...
                    debug!(
                        "send_encrypted_request_to_node: Not an envelope ({}), server likely sent unencrypted postcard response",
                        response_bytes.len()
                    );
                }
            }
    ...
    pub async fn send_encrypted_request(
        ...
        // Treat 405 Method Not Allowed specially: some API clients POST envelope payloads
        // and expect the server to return a body even when the server doesn't consider
        // the request a 'success' status. In that case we return the response body bytes
        ...
        if response_bytes.len() > 100 && self.machine_public_key.is_some() {
            ...
            match protocol::machine::root_as_envelope(&response_bytes) {
                ...
                Err(e) => {
                    log::debug!(
                        "send_encrypted_request: Not an envelope ({}), server likely sent unencrypted postcard response",
                        e
                    );
                    // Server sent unencrypted postcard response directly, return as-is
                    Ok(response_bytes.to_vec())
                }
            }
        } else {
