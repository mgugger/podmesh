...
const UNSUPPORTED_KINDS: &[(&str, &str)] = &[
    (
        ...
        "PVC not supported — podmesh has no distributed storage layer",
    ),
    (
        "HorizontalPodAutoscaler",
        "HPA not supported — use podmesh.io/replicas annotation",
    ),
    (
        "DaemonSet",
        "DaemonSet not supported — podmesh uses rendezvous placement",
    ),
];
...
pub fn convert_manifest(yaml_str: &str) -> anyhow::Result<(String, Vec<String>)> {
    ...
    for doc in serde_yaml::Deserializer::from_str(yaml_str) {
        ...
        if kind == "Service"
            && let Some(svc_type) = value
                .get("spec")
                .and_then(|s| s.get("type"))
                .and_then(|t| t.as_str())
            && UNSUPPORTED_SERVICE_TYPES.contains(&svc_type)
        {
        ...
        if kind == "Secret" {
            warnings.push(
                "WARNING: Secret — seal secrets inside the workload spec using podmesh encryption"
                    .to_string(),
            );
        }
...
"#;
    ...
    fn test_convert_warns_on_hpa() {
        ...
        assert!(
            warnings
                .iter()
                .any(|w| w.contains("HorizontalPodAutoscaler"))
        );
    }
