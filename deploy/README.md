...
```
The rootless stack now consists of two pods: `machine` (hosts the scheduler bootstrap and workers)
and `workload-bootstrap` (hosts the podmesh-proxy plus the additional workload peers). Both pods must
